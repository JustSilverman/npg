...
import { parseValue } from '../../../src/pg-type-parser'

describe('Type parser accepts a buffer and returns a JavaScript type', () => {
  describe('unsupported types', () => {
    it('returns the value as a string', () => {
      const unsupportedTypeId = -23
      const rowData = new Buffer('some string', 'utf8')
      assert.deepEqual(parseValue(unsupportedTypeId, 'utf8', rowData), 'some string')
    })
  })
  ...
  describe('integer types', () => {
    describe('int2', () => {
      it('parses a positive 2 byte integer', () => {
        ...
        assert.deepEqual(parseValue(21, 'utf8', rowData), 1)
      })
    ...
    describe('int4', () => {
      it('parses a positive 4 byte integer', () => {
        ...
        assert.deepEqual(parseValue(23, 'utf8', rowData), 2147483647)
      })
    ...
    describe('int8', () => {
      ...
      it('parses a positive 8 byte integer', () => {
        ...
        assert.deepEqual(parseValue(20, 'utf8', rowData), 9223372036854775807)
      })
  ...
  describe('floating point types', () => {
    describe('real type', () => {
      ...
      it('parses a positive float4', () => {
        const rowData = asBuffer
        assert.deepEqual(parseValue(700, 'utf8', rowData), 123.456)
      })
    ...
    describe('double precision type', () => {
      ...
      it('parses a positive float8', () => {
        const rowData = asBuffer
        assert.deepEqual(parseValue(701, 'utf8', rowData), 12345678.12345678)
      })
  ...
  describe('Arbitrary precision types', () => {
    describe('numeric (aka decimal) type', () => {
      ...
      it('parses a positive bignum', () => {
        const rowData = asBuffer
        assert.deepEqual(parseValue(1700, 'utf8', rowData), bignumAsNum)
      })

      it('parses a negative bignum', () => {
        const rowData = Buffer.concat([new Buffer([0x2d]), asBuffer])
        assert.deepEqual(parseValue(1700, 'utf8', rowData), -1 * bignumAsNum)
      })
    })
  })
  ...
  describe('Booleans', () => {
    it('parses a boolean and is true for all true states', () => {
      ['TRUE', 't', 'true', 'y', 'yes', 'on', '1'].forEach((value) => {
        assert.strictEqual(parseValue(16, 'utf8', new Buffer(value, 'utf8')), true)
      })
    })

    it('parses a boolean and is false for all false states', () => {
      ['FALSE', 'f', 'false', 'n', 'no', 'off', '0'].forEach((value) => {
        assert.strictEqual(parseValue(16, 'utf8', new Buffer(value, 'utf8')), false)
      })
    })

    it('throws if passed an unsupported boolean value', () => {
      assert.throws(() => {
        parseValue(16, 'utf8', new Buffer('very true', 'utf8'))
      })
    })
  })
})
